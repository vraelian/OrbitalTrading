...
export class UIManager {
    ...
    updateMarketScreen(gameState) {
        ...
        availableCommodities.forEach(good => {
            ...
            if (!cardContainer) return; // Skip if card not in DOM
    ...
    renderHangarScreen(gameState) {
        const { player, currentLocationId, market } = gameState;

        // --- CHANGE START ---
        // Shipyard Content - Read from the pre-generated stock in the game state
        const shipsForSaleIds = market.shipyardStock[currentLocationId]?.shipsForSale || [];
        const shipsForSale = shipsForSaleIds
            .map(id => ([id, SHIPS[id]]))
            .filter(([id, ship]) => !player.ownedShipIds.includes(id)); // Final check to prevent showing owned ships
