...
export class EventManager {
    ...
    _handleClick(e) {
        ...
        if (this.uiManager.isMobile) {
            ...
            // If a tooltip is currently active
            if (this.activeTooltipTarget) {
                const isClickingSameTarget = this.activeTooltipTarget === newTarget;
                // Always hide the currently active tooltip first
                this.uiManager.hideGraph();
                this.uiManager.hideGenericTooltip();
                this.activeTooltipTarget = null;
                
                // If the user clicked the same element that was already active, we're done. Just close it.
                if (isClickingSameTarget) {
                    return;
                }
            }
            
            // If we clicked a new, valid target, show its tooltip
            if (newTarget) {
    ...
    _startRefueling(buttonElement) {
        ...
        this.refuelInterval = setInterval(() => this._refuelTick(buttonElement), 500); // 2 ticks per second
    }
    // --- CHANGE END ---
    ...
    _startRepairing(buttonElement) {
        ...
        this.repairInterval = setInterval(() => this._repairTick(buttonElement), 500); // 2 ticks per second
    }
    // --- CHANGE END ---
