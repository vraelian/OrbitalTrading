...
Avg. Cost: ${g(r.avgCost,!1)}`;return`
    ...
    `}function xe(){return`
        <div class="text-center p-8 flex flex-col items-center gap-4">
            ...
            </div>`,x=Object.keys(this.navStructure).map(S=>{let C=S===a,I=o[S]||Object.keys(this.navStructure[S].screens)[0],w=h&&h.navId!==S,$=n||w,D=C?`background: ${T.gradient}; color: ${T.textColor};`:"";return`<div class="tab ${S}-tab ${C?"active":""} ${$?"disabled":""}" style="${D}" data-action="${b.SET_SCREEN}" data-nav-id="${S}" data-screen-id="${I}">${this.navStructure[S].label}</div>`}).join(""),_="";if(p&&m&&f){let S=O(f),C=m.health/p.maxHealth*100,I=m.fuel/p.maxFuel*100,w=S/p.cargoCapacity*100;_=`
